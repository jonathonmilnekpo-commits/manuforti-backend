#!/usr/bin/env python3
"""Generate WEG S.A. Product 1 v15 — Full Pipeline"""
import sys
...
VISUALS = BASE / 'weg_v16_fixed_visuals'
VISUALS.mkdir(exist_ok=True)
CLAN_LOGO = BASE / 'skills/product-1-generator/assets/manu_forti_logo.png'
OUTPUT = BASE / 'WEG_Product1_v16_FIXED.pptx'
...
def gen_risk_gauge():
    ...
    ax.text(0, 1.25, 'WEG S.A. - Overall Risk Assessment', ha='center', fontsize=18, fontweight='bold', color=M_NAVY)
    ax.text(0, 1.10, 'Hydrogen Electrolysers | Alkaline & PEM | Global Manufacturing', ha='center', fontsize=12, color=M_GREY)
    boxes = [('Financial', 'MEDIUM', M_AMBER), ('Operational', 'LOW', M_GREEN), ('Geopolitical', 'LOW', M_GREEN), ('ESG', 'LOW', M_GREEN)]
...
def gen_org_chart():
    ...
    ax.add_patch(parent)
    ax.text(8, 8.1, 'WEG S.A. (Bovespa: WEGE3)', ha='center', fontsize=18, fontweight='bold', color='white')
    ax.text(8, 7.75, 'Jaraguá do Sul, Brazil, Norway | Public Company | Founded 1961', ha='center', fontsize=10, color='#cccccc')
    divs = [('Alkaline\nElectrolyser', 'Heroya, Norway\n1 GW Capacity', 1.0), ('PEM\nElectrolyser', 'Wallingford, CT\n500 MW + GM Partnership', 4.5), ('US\nGigafactory', '4 GW Planned\nLocation TBD', 8.0), ('R&D &\nCorporate', 'Jaraguá do Sul, Brazil HQ\nGlobal Sales', 11.5)]
    ...
    facts = [('NOK 1.6B\nOrder Backlog', 1.5, 2.8), ('~40,000\nEmployees', 5.0, 2.8), ('NOK 1.9B\nCash', 8.5, 2.8), ('Listed on\nJaraguá do Sul, Brazil Bors', 12.0, 2.8)]
...
def gen_financial():
    ...
    revenue = [796, 1064, 1253, 1400]
    ebitda = [-120, -250, -310, -350]
    margin = [-15.1, -23.5, -24.7, -25.0]
    ...
    ax1.set_ylim(0, 1800); ax1.tick_params(axis='y', labelcolor=M_NAVY, labelsize=11)
    ...
    for x, y, m in zip(years, ebitda, margin):
        ax2.text(x, y-40, f'NOK {y}M\n({m:.1f}%)', ha='center', fontsize=11, fontweight='bold', color=M_RED)
    ax2.set_ylabel('EBITDA (NOK Millions)', fontsize=13, color=M_RED, fontweight='bold')
    ax2.set_ylim(-500, 0); ax2.tick_params(axis='y', labelcolor=M_RED, labelsize=11)
    ax1.spines['top'].set_visible(False); ax2.spines['top'].set_visible(False)
    ax1.grid(axis='y', alpha=0.2, linestyle='--')
    ax1.set_title('Revenue & EBITDA Trajectory 2021-2024E', fontsize=16, fontweight='bold', color=M_NAVY, pad=15)
    props = dict(boxstyle='round,pad=0.5', facecolor=M_LIGHT, edgecolor=M_STEEL, linewidth=1.5)
    ax1.text(0.02, 0.97, 'Revenue growing but EBITDA remains negative.\nScale-up phase with heavy R&D and manufacturing\ninvestment. Path to profitability expected 2026-27.', transform=ax1.transAxes, fontsize=11, va='top', bbox=props, color=M_NAVY)
...
def gen_market():
    fig, ax = plt.subplots(figsize=(16, 9), facecolor='white')
    companies = ['Thyssenkrupp Nucera', 'Siemens Energy', 'John Cockerill', 'WEG S.A.', 'ITM Power', 'Plug Power']
    capacity = [3000, 2500, 2000, 1500, 800, 600]
    ...
    for bar, val, c in zip(bars, capacity, companies):
        lc = 'white' if c == 'WEG S.A.' else M_NAVY
    ...
    ax.annotate('Nel: 1 GW Heroya + 500 MW Wallingford\n+ 4 GW US Gigafactory planned', xy=(1500, 3), xytext=(2200, 1.5), fontsize=11, fontweight='bold', color=M_STEEL, arrowprops=dict(arrowstyle='->', color=M_STEEL, lw=2))
    plt.tight_layout()
...
def gen_risk_matrix():
    ...
    ax.set_title('Risk Matrix - WEG S.A.', fontsize=14, fontweight='bold', color=M_NAVY, pad=10)
...
def gen_radar():
    ...
    ax.plot(ang_c, nel_c, color=M_NAVY, linewidth=2.5, marker='o', markersize=8, label='WEG S.A.')
...
def gen_peer_risk():
    ...
    companies = ['WEG S.A.', 'Siemens Energy', 'Thyssenkrupp', 'ITM Power', 'Plug Power', 'Bloom Energy']
    scores = [48, 32, 35, 55, 68, 52]
    ...
    ax.scatter(0, 48, s=350, color=M_NAVY, zorder=4, edgecolors=M_NAVY, linewidth=2)
    ax.text(0, 51, '48', ha='center', fontsize=14, fontweight='bold', color=M_NAVY)
...
def gen_esg():
    ...
    fig.suptitle('ESG Assessment - WEG S.A.', fontsize=22, fontweight='bold', color=M_NAVY, y=0.98)
    ...
    pillars = [
        ...
        {'title': 'Social', 'rating': 'LOW', 'color': M_GREEN, 'items': [('Brazilian Labour Standards', M_GREEN, '✓'), ('Employee Safety (Industrial)', M_GREEN, '✓'), ('Green Jobs Creation', M_GREEN, '✓'), ('US Manufacturing Expansion', M_GREEN, '✓'), ('Diversity Reporting', M_AMBER, '⚠'), ('Community Engagement', M_AMBER, '⚠')]},
        {'title': 'Governance', 'rating': 'MEDIUM', 'color': M_AMBER, 'items': [('Public Company Disclosure', M_GREEN, '✓'), ('Jaraguá do Sul, Brazil Bors Listed', M_GREEN, '✓'), ('Board Independence', M_GREEN, '✓'), ('Executive Compensation', M_AMBER, '⚠'), ('Shareholder Structure', M_AMBER, '⚠'), ('Cavendish Spin-off', M_AMBER, '⚠')]}
    ]
...
print("Generating WEG S.A. Product 1 v15...")
print("Step 1: Charts...")
visuals = {
...
add_text(s, 0.7, 2.05, 12, 1.2, 'WEG S.A.', 44, True, WHITE)
add_text(s, 0.7, 3.5, 12, 0.6, 'Hydrogen Electrolysers | Alkaline & PEM | Global Manufacturing', 18, False, (200,210,220))
...
add_text(s, 0.7, 4.55, 12, 0.4, 'R$ 38.0B Revenue | ~40,000 Employees | Jaraguá do Sul, Brazil Bors Listed | Founded 1961', 14, False, (200,210,220))
...
s = prs.slides.add_slide(blank)
add_header(s, 'EXECUTIVE SUMMARY', 'WEG S.A. - Risk & Suitability Overview')
...
snap = [('Supplier:', 'WEG S.A. (Bovespa: WEGE3)'), ('Type:', 'Public Company'), ('Sector:', 'Hydrogen Electrolysers'), ('HQ:', 'Jaraguá do Sul, Brazil, Norway'), ('Founded:', '1961'), ('Employees:', '~40,000'), ('Revenue:', 'NOK 1.3B (2023)'), ('Order Backlog:', 'NOK 1.6B'), ('Cash:', 'NOK 1.9B'), ('Manufacturing:', '1.5 GW Capacity')]
...
findings = ['• Leading Nordic electric motors & automation manufacturer with 1+ GW capacity', '• Dual technology: Alkaline (Heroya, Norway) and PEM (Wallingford, US)', '• Negative EBITDA but strong cash position (NOK 1.9B) and order backlog', '• ESG leader: Enabling green hydrogen economy; strong environmental profile']
...
add_bullets(s, 0.5, 2.65, 6, 2.5, 'Commercial Conditions', ['Payment: Milestone-based (no advance payment)', 'Performance bonds: 10-15% of contract value', 'Liquidated damages: For delivery delays', 'Parent guarantee: Not applicable (public company)', 'Local entity: Brazilian contract preferred'], 14, 11)
...
for x, t, d in [(0.6,'Financial: MEDIUM','Negative EBITDA; strong cash position'), (3.7,'Operational: LOW','Proven manufacturing; 1+ GW capacity'), (6.8,'Geopolitical: LOW','Brazilian-based; stable jurisdiction'), (9.9,'ESG: LOW','Green hydrogen enabler; strong profile')]:
...
add_bullets(s, 8.7, 1.05, 4.3, 3.5, 'Company Overview', ['WEG S.A. is a leading global hydrogen', 'company specializing in electrolyser', 'technology for production of green', 'hydrogen from renewable energy.', '', 'Founded 1961; spun out as dedicated', 'hydrogen company in 2015. Listed on', 'Jaraguá do Sul, Brazil Bors (Bovespa: WEGE3).'], 14, 11)
add_bullets(s, 8.7, 4.5, 4.3, 2.4, 'Leadership & Operations', ['CEO: Hakon Volldal', '  (appointed 2020)', '', 'HQ: Jaraguá do Sul, Brazil, Norway', 'Manufacturing: Heroya (NO),', '  Wallingford (US)', '', 'R&D: GM partnership (PEM)'], 14, 11)
add_source(s); add_logos(s)

# S5: FINANCIAL HEALTH
print("  Slide 5: Financial Health")
s = prs.slides.add_slide(blank)
add_header(s, 'FINANCIAL HEALTH', 'Revenue & EBITDA Trajectory 2021-2024E | Listed on Jaraguá do Sul, Brazil Bors')
...
metrics = [('2023 Revenue:', 'NOK 1,253M'), ('2024E Revenue:', 'NOK ~1,400M'), ('Revenue Growth:', '+21% YoY (Q3 2024)'), ('EBITDA:', 'NOK -90M (Q3 2024)'), ('Order Backlog:', 'NOK 1,614M (Q4 2024)'), ('Cash Balance:', 'NOK 1,876M'), ('Employees:', '~40,000'), ('Market Cap:', 'NOK ~15B (varies)')]
for idx, (label, val) in enumerate(metrics):
    y = 1.56 + idx * 0.30
    add_text(s, 8.1, y, 2.55, 0.29, label, 11, True, TEXT_SECONDARY)
    vc = AMBER if 'EBITDA' in label else TEXT_PRIMARY
...
add_bullets(s, 8.5, 3.2, 4.5, 2.6, 'Nel Competitive Advantages', ['1+ GW manufacturing capacity', 'Dual technology (Alkaline + PEM)', 'Brazilian/Nordic green energy access', 'US expansion (4 GW Gigafactory planned)', 'GM partnership for PEM technology', 'Public company transparency'], 14, 11)
