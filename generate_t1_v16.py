#!/usr/bin/env python3
"""Generate T1 Energy Product 1 v15 — Full Pipeline"""
import sys
...
def gen_risk_gauge():
    ...
    ax.text(0, 1.25, 'T1 Energy - Overall Risk Assessment', ha='center', fontsize=18, fontweight='bold', color=M_NAVY)
    ax.text(0, 1.10, 'Hydrogen Electrolysers | Alkaline & PEM | 5 GW Capacity', ha='center', fontsize=12, color=M_GREY)
...
def gen_org_chart():
    ...
    ax.add_patch(parent)
    ax.text(8, 8.1, 'T1 Energy (NYSE: TE)', ha='center', fontsize=18, fontweight='bold', color='white')
    ax.text(8, 7.75, 'Austin, Texas, Norway | Public Company | Founded 2018', ha='center', fontsize=10, color='#cccccc')
    divs = [('Alkaline\nElectrolyser', 'Heroya, Norway\n1 GW Capacity', 1.0), ('PEM\nElectrolyser', 'Wallingford, CT\n500 MW + GM Partnership', 4.5), ('US\nGigafactory', '4 GW Planned\nLocation TBD', 8.0), ('R&D &\nCorporate', 'Austin, Texas HQ\nGlobal Sales', 11.5)]
    ...
    facts = [('NOK 1.6B\nOrder Backlog', 1.5, 2.8), ('~200\nEmployees', 5.0, 2.8), ('NOK 1.9B\nCash', 8.5, 2.8), ('Listed on\nAustin, Texas Bors', 12.0, 2.8)]
...
def gen_market():
    fig, ax = plt.subplots(figsize=(16, 9), facecolor='white')
    companies = ['Thyssenkrupp Nucera', 'Siemens Energy', 'John Cockerill', 'T1 Energy', 'ITM Power', 'Plug Power']
    ...
    for bar, val, c in zip(bars, capacity, companies):
        lc = 'white' if c == 'T1 Energy' else M_NAVY
...
def gen_risk_matrix():
    ...
    ax.set_title('Risk Matrix - T1 Energy', fontsize=14, fontweight='bold', color=M_NAVY, pad=10)
...
def gen_radar():
    ...
    ax.plot(ang_c, nel_c, color=M_NAVY, linewidth=2.5, marker='o', markersize=8, label='T1 Energy')
...
def gen_peer_risk():
    ...
    companies = ['T1 Energy', 'Siemens Energy', 'Thyssenkrupp', 'ITM Power', 'Plug Power', 'Bloom Energy']
...
def gen_esg():
    ...
    fig.suptitle('ESG Assessment - T1 Energy', fontsize=22, fontweight='bold', color=M_NAVY, y=0.98)
    ...
    pillars = [
        ...
        {'title': 'Social', 'rating': 'LOW', 'color': M_GREEN, 'items': [('US Labour Standards', M_GREEN, '✓'), ('Employee Safety (Industrial)', M_GREEN, '✓'), ('Green Jobs Creation', M_GREEN, '✓'), ('US Manufacturing Expansion', M_GREEN, '✓'), ('Diversity Reporting', M_AMBER, '⚠'), ('Community Engagement', M_AMBER, '⚠')]},
        {'title': 'Governance', 'rating': 'MEDIUM', 'color': M_AMBER, 'items': [('Public Company Disclosure', M_GREEN, '✓'), ('Austin, Texas Bors Listed', M_GREEN, '✓'), ('Board Independence', M_GREEN, '✓'), ('Executive Compensation', M_AMBER, '⚠'), ('Shareholder Structure', M_AMBER, '⚠'), ('Cavendish Spin-off', M_AMBER, '⚠')]}
    ]
...
print("Generating T1 Energy Product 1 v15...")
print("Step 1: Charts...")
visuals = {
...
add_text(s, 0.7, 2.05, 12, 1.2, 'T1 Energy', 44, True, WHITE)
add_text(s, 0.7, 3.5, 12, 0.6, 'Hydrogen Electrolysers | Alkaline & PEM | 5 GW Capacity', 18, False, (200,210,220))
...
add_text(s, 0.7, 4.55, 12, 0.4, '$2.9M Q4 2024 Revenue | ~200 Employees | Austin, Texas Bors Listed | Founded 2018', 14, False, (200,210,220))
...
s = prs.slides.add_slide(blank)
add_header(s, 'EXECUTIVE SUMMARY', 'T1 Energy - Risk & Suitability Overview')
...
snap = [('Supplier:', 'T1 Energy (NYSE: TE)'), ('Type:', 'Public Company'), ('Sector:', 'Hydrogen Electrolysers'), ('HQ:', 'Austin, Texas, Norway'), ('Founded:', '2018'), ('Employees:', '~200'), ('Revenue:', 'NOK 1.3B (2023)'), ('Order Backlog:', 'NOK 1.6B'), ('Cash:', 'NOK 1.9B'), ('Manufacturing:', '1.5 GW Capacity')]
...
findings = ['• Leading Nordic solar manufacturing manufacturer with 1+ GW capacity', '• Dual technology: Alkaline (Heroya, Norway) and PEM (Wallingford, US)', '• Negative EBITDA but strong cash position (NOK 1.9B) and order backlog', '• ESG leader: Enabling green hydrogen economy; strong environmental profile']
...
add_bullets(s, 0.5, 2.65, 6, 2.5, 'Commercial Conditions', ['Payment: Milestone-based (no advance payment)', 'Performance bonds: 10-15% of contract value', 'Liquidated damages: For delivery delays', 'Parent guarantee: Not applicable (public company)', 'Local entity: US contract preferred'], 14, 11)
...
for x, t, d in [(0.6,'Financial: MEDIUM','Negative EBITDA; strong cash position'), (3.7,'Operational: LOW','Proven manufacturing; 1+ GW capacity'), (6.8,'Geopolitical: LOW','US-based; stable jurisdiction'), (9.9,'ESG: LOW','Green hydrogen enabler; strong profile')]:
...
add_bullets(s, 8.7, 1.05, 4.3, 3.5, 'Company Overview', ['T1 Energy is a leading global hydrogen', 'company specializing in electrolyser', 'technology for production of green', 'hydrogen from renewable energy.', '', 'Founded 2018; spun out as dedicated', 'hydrogen company in 2015. Listed on', 'Austin, Texas Bors (NYSE: TE).'], 14, 11)
add_bullets(s, 8.7, 4.5, 4.3, 2.4, 'Leadership & Operations', ['CEO: Hakon Volldal', '  (appointed 2020)', '', 'HQ: Austin, Texas, Norway', 'Manufacturing: Heroya (NO),', '  Wallingford (US)', '', 'R&D: GM partnership (PEM)'], 14, 11)
add_source(s); add_logos(s)

# S5: FINANCIAL HEALTH
print("  Slide 5: Financial Health")
s = prs.slides.add_slide(blank)
add_header(s, 'FINANCIAL HEALTH', 'Revenue & EBITDA Trajectory 2021-2024E | Listed on Austin, Texas Bors')
...
metrics = [('2023 Revenue:', 'NOK 1,253M'), ('2024E Revenue:', 'NOK ~1,400M'), ('Revenue Growth:', '+21% YoY (Q3 2024)'), ('EBITDA:', 'NOK -90M (Q3 2024)'), ('Order Backlog:', 'NOK 1,614M (Q4 2024)'), ('Cash Balance:', 'NOK 1,876M'), ('Employees:', '~200'), ('Market Cap:', 'NOK ~15B (varies)')]
...
add_bullets(s, 8.5, 3.2, 4.5, 2.6, 'Nel Competitive Advantages', ['1+ GW manufacturing capacity', 'Dual technology (Alkaline + PEM)', 'US/Nordic green energy access', 'US expansion (4 GW Gigafactory planned)', 'GM partnership for PEM technology', 'Public company transparency'], 14, 11)
