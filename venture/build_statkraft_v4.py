...
T(s,"From Framework to Agentic Capability — Powered by Nvidia NemoClaw + Nemotron",0.6,2.4,12,0.55,sz=22,color=GREY)
LINE(s,3.1,STEEL)
...
T(s,"Prepared by Aiden  |  March 17, 2026  |  Confidential",
...
pts=[
    ...
    "Nemotron models: purpose-built for agentic reasoning, open-weight, on-prem",
...
implications=[
    ...
    ("The model cost is zero","Nemotron models are open-weight. Inference cost: €0/token on-prem."),
...
agents=[
    ...
    ("🗺️","Category\nStrategist",AMBER,
     ...
     "Weeks → days. Highest-value agent — internal or commercial deployment."),
    ("🤝","Negotiation\nPrep",TEAL,
...
layers=[
    ("L1","MODEL DEPLOYMENT",STEEL,
     "Nemotron 3 Super (120B/12B active) + Nemotron Mini 4B on-prem via NemoClaw",
    ...
    ("L5","PROMPT INJECTION DEFENCE",RED,
     ...
     "Software: Custom sanitisation pipeline (Python)  |  Separate parsing model (Nemotron Mini 4B)  |  Cost: ~€5K build"),
...
tiers=[
    ("TIER 1\nOn-Premises",RED,"RESTRICTED + CONFIDENTIAL",[
        "Models: Nemotron 3 Super (120B/12B active) / Nemotron Mini 4B","Inference: NemoClaw (OpenShell) + vLLM",
        "Orchestration: NemoClaw + LangChain","Vector DB: ChromaDB / Qdrant (on-prem)",
    ...
    ("TIER 2\nPrivate Cloud",AMBER,"INTERNAL data — controlled VPC",[
        ...
        "Cost: ~€2K/month (~€24K/yr)","Models: GPT-4o, Claude Sonnet, or Nemotron via Nvidia API (API)",
...
T(s,"Prepared by Aiden  |  For Statkraft Procurement  |  March 17, 2026",
  0.6,5.68,12,0.4,sz=14,bold=True,color=WHITE)
T(s,"procurement.ai@statkraft.com  |  statkraft.com",
  0.6,6.12,12,0.35,sz=12,color=WHITE,italic=True)

out="/Users/jonathonmilne/.openclaw/workspace/venture/Statkraft_Procurement_AI_Strategy_v4_March2026.pptx"
prs.save(out)
print(f"Saved: {out}")
